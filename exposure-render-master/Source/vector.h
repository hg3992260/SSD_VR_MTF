...
static inline HOST_DEVICE float AbsDot(const Vec3f& A, const Vec3f& B)						{ return fabsf(A.Dot(B));													};
static inline HOST_DEVICE float ClampedAbsDot(const Vec3f& A, const Vec3f& B)				{ return Clamp(fabsf(A.Dot(B)), 0.0f, 1.0f);									};
